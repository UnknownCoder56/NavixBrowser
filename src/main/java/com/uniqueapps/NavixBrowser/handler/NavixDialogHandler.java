...
public class NavixDialogHandler implements CefDialogHandler {
    ...
    public boolean onFileDialog(CefBrowser cefBrowser, FileDialogMode fileDialogMode, String title, String defaultFilePath, Vector<String> acceptFilters, int selectedAcceptFilter, CefFileDialogCallback cefFileDialogCallback) {
        ...
        if (acceptFilters.stream().anyMatch(s -> s.contains("/"))) {
            ...
            chooser.setFileFilter(new FileNameExtensionFilter("Supported files " + acceptFilters, acceptFilters.stream().filter(s -> s.contains(".")).toArray(String[]::new)));
        } else {
        ...
        if (fileDialogMode == FileDialogMode.FILE_DIALOG_OPEN) {
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                cefFileDialogCallback.Continue(0, new Vector<>(selectedPaths));
                success = true;
            }
